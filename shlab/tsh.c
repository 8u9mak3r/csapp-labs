...
void sigchld_handler(int sig)
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)
    {
        ...
        /* assume that waitpid always returns a terminated pid prior to a stopped one*/
        /* waitpid returning a stopped pid indicates that terminated pids 
        are either all reaped or not even in existence
        */
